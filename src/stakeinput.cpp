...
// The zDID block index is the first appearance of the accumulator checksum that was used in the spend
...
{
    //The unique identifier for a zDID is a hash of the serial
...
{
    //Create an output returning the zDID that was staked
    CTxOut outReward;
    libzerocoin::CoinDenomination denomStaked = libzerocoin::AmountToZerocoinDenomination(this->GetValue());
    CDeterministicMint dMint;
    if (!pwallet->CreateZDIDOutPut(denomStaked, outReward, dMint))
        return error("%s: failed to create zDID output", __func__);
    vout.emplace_back(outReward);

    //Add new staked denom to our wallet
    if (!pwallet->DatabaseMint(dMint))
        return error("%s: failed to database the staked zDID", __func__);

    for (unsigned int i = 0; i < 3; i++) {
        CTxOut out;
        CDeterministicMint dMintReward;
        if (!pwallet->CreateZDIDOutPut(libzerocoin::CoinDenomination::ZQ_ONE, out, dMintReward))
            return error("%s: failed to create zDID output", __func__);
...
{
    CzDIDTracker* zdidTracker = pwallet->zdidTracker.get();
    CMintMeta meta;
    if (!zdidTracker->GetMetaFromStakeHash(hashSerial, meta))
        return error("%s: tracker does not have serialhash", __func__);

    zdidTracker->SetPubcoinUsed(meta.hashPubcoin, txid);
    return true;
}
...
{
    //The unique identifier for a DID stake is the outpoint
