...
CzDIDWallet::CzDIDWallet(std::string strWalletFile)
{
    ...
    //Check for old db version of storing zdid seed
    if (fFirstRun) {
        uint256 seed;
        if (walletdb.ReadZDIDSeed_deprecated(seed)) {
            //Update to new format, erase old
            seedMaster = seed;
            hashSeed = Hash(seed.begin(), seed.end());
            if (pwalletMain->AddDeterministicSeed(seed)) {
                if (walletdb.EraseZDIDSeed_deprecated()) {
                    LogPrintf("%s: Updated zDID seed databasing\n", __func__);
                    fFirstRun = false;
                } else {
                    LogPrintf("%s: failed to remove old zdid seed\n", __func__);
                }
            }
        }
    }
    ...
    if (fFirstRun) {
        ...
        seedMaster = seed;
        LogPrintf("%s: first run of zdid wallet detected, new seed generated. Seedhash=%s\n", __func__, Hash(seed.begin(), seed.end()).GetHex());
...
bool CzDIDWallet::SetMasterSeed(const uint256& seedMaster, bool fResetCount)
{
    ...
    if (fResetCount)
        walletdb.WriteZDIDCount(nCountLastUsed);
    else if (!walletdb.ReadZDIDCount(nCountLastUsed))
        nCountLastUsed = 0;
...
void CzDIDWallet::Lock()
{
    seedMaster = 0;
}

void CzDIDWallet::AddToMintPool(const std::pair<uint256, uint32_t>& pMint, bool fVerbose)
{
    mintPool.Add(pMint, fVerbose);
}

//Add the next 20 mints to the mint pool
void CzDIDWallet::GenerateMintPool(uint32_t nCountStart, uint32_t nCountEnd)
{
    ...
    for (uint32_t i = n; i < nStop; ++i) {
        ...
        CKey key;
        SeedToZDID(seedZerocoin, bnValue, bnSerial, bnRandomness, key);
...
bool CzDIDWallet::LoadMintPoolFromDB()
{
...
void CzDIDWallet::RemoveMintsFromPool(const std::vector<uint256>& vPubcoinHashes)
{
    for (const uint256& hash : vPubcoinHashes)
        mintPool.Remove(hash);
}

void CzDIDWallet::GetState(int& nCount, int& nLastGenerated)
{
...
void CzDIDWallet::SyncWithChain(bool fGenerateMintPool)
{
    ...
    while (found) {
        ...
        for (pair<uint256, uint32_t> pMint : listMints) {
            ...
            if (pwalletMain->zdidTracker->HasPubcoinHash(pMint.first)) {
...
bool CzDIDWallet::SetMintSeen(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const CoinDenomination& denom)
{
    ...
    CBigNum bnRandomness;
    CKey key;
    SeedToZDID(seedZerocoin, bnValueGen, bnSerial, bnRandomness, key);
    ...
    // Add to zdidTracker which also adds to database
    pwalletMain->zdidTracker->Add(dMint, true);
...
void CzDIDWallet::SeedToZDID(const uint512& seedZerocoin, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key)
{
...
uint512 CzDIDWallet::GetZerocoinSeed(uint32_t n)
{
...
void CzDIDWallet::GenerateDeterministicZDID(CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly)
{
...
void CzDIDWallet::GenerateMint(const uint32_t& nCount, const CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint)
{
...
bool CzDIDWallet::RegenerateMint(const CDeterministicMint& dMint, CZerocoinMint& mint)
{
