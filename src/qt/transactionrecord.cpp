...
QList<TransactionRecord> TransactionRecord::decomposeTransaction(const CWallet* wallet, const CWalletTx& wtx)
{
    ...
    if (wtx.IsCoinStake()) {
        ...
        if (wtx.IsZerocoinSpend() && (fZSpendFromMe || wallet->zdidTracker->HasMintTx(hash))) {
            //zDID stake reward
            sub.involvesWatchAddress = false;
            sub.type = TransactionRecord::StakeZDID;
        ...
        BOOST_FOREACH (const CTxOut& txout, wtx.vout) {
            ...
            if (mine) {
                ...
                if (ExtractDestination(txout.scriptPubKey, address) && IsMine(*wallet, address)) {
                    // Received by DIRTYDEEDS Address
...
bool IsZDIDType(TransactionRecord::Type type)
{
    switch (type) {
        case TransactionRecord::StakeZDID:
...
void TransactionRecord::updateStatus(const CWalletTx& wtx)
{
    ...
    else if (type == TransactionRecord::Generated || type == TransactionRecord::StakeMint || type == TransactionRecord::StakeZDID || type == TransactionRecord::MNReward) {
