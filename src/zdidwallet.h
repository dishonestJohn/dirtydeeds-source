...
#ifndef DIRTYDEEDS_ZDIDWALLET_H
#define DIRTYDEEDS_ZDIDWALLET_H
...
public:
    CzDIDWallet(std::string strWalletFile);
    ...
    void GenerateDeterministicZDID(libzerocoin::CoinDenomination denom, libzerocoin::PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly = false);
    ...
    void UpdateCount();
    void Lock();
    void SeedToZDID(const uint512& seed, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key);

private:
    uint512 GetZerocoinSeed(uint32_t n);
};

#endif //DIRTYDEEDS_ZDIDWALLET_H
