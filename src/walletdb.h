...
public:
    ...
    bool WriteZDIDSeed(const uint256& hashSeed, const vector<unsigned char>& seed);
    bool ReadZDIDSeed(const uint256& hashSeed, vector<unsigned char>& seed);
    bool ReadZDIDSeed_deprecated(uint256& seed);
    bool EraseZDIDSeed();
    bool EraseZDIDSeed_deprecated();

    bool WriteZDIDCount(const uint32_t& nCount);
    bool ReadZDIDCount(uint32_t& nCount);
