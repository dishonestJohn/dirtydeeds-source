...
CzDIDTracker::CzDIDTracker(std::string strWalletFile)
{
...
CzDIDTracker::~CzDIDTracker()
{
    mapSerialHashes.clear();
    mapPendingSpends.clear();
}

void CzDIDTracker::Init()
{
...
bool CzDIDTracker::Archive(CMintMeta& meta)
{
...
bool CzDIDTracker::UnArchive(const uint256& hashPubcoin, bool isDeterministic)
{
    CWalletDB walletdb(strWalletFile);
    if (isDeterministic) {
        CDeterministicMint dMint;
...
CMintMeta CzDIDTracker::Get(const uint256 &hashSerial)
{
...
CMintMeta CzDIDTracker::GetMetaFromPubcoin(const uint256& hashPubcoin)
{
...
bool CzDIDTracker::GetMetaFromStakeHash(const uint256& hashStake, CMintMeta& meta) const
{
    for (auto& it : mapSerialHashes) {
...
std::vector<uint256> CzDIDTracker::GetSerialHashes()
{
...
CAmount CzDIDTracker::GetBalance(bool fConfirmedOnly, bool fUnconfirmedOnly) const
{
    CAmount nTotal = 0;
...
CAmount CzDIDTracker::GetUnconfirmedBalance() const
{
    return GetBalance(false, true);
}

std::vector<CMintMeta> CzDIDTracker::GetMints(bool fConfirmedOnly) const
{
...
bool CzDIDTracker::HasMintTx(const uint256& txid)
{
    for (auto it : mapSerialHashes) {
...
bool CzDIDTracker::HasPubcoin(const CBigNum &bnValue) const
{
...
bool CzDIDTracker::HasPubcoinHash(const uint256& hashPubcoin) const
{
    for (auto it : mapSerialHashes) {
        CMintMeta meta = it.second;
        if (meta.hashPubcoin == hashPubcoin)
...
bool CzDIDTracker::HasSerial(const CBigNum& bnSerial) const
{
    uint256 hash = GetSerialHash(bnSerial);
    return HasSerialHash(hash);
}

bool CzDIDTracker::HasSerialHash(const uint256& hashSerial) const
{
    auto it = mapSerialHashes.find(hashSerial);
    return it != mapSerialHashes.end();
}

bool CzDIDTracker::UpdateZerocoinMint(const CZerocoinMint& mint)
{
...
bool CzDIDTracker::UpdateState(const CMintMeta& meta)
{
    CWalletDB walletdb(strWalletFile);
...
void CzDIDTracker::Add(const CDeterministicMint& dMint, bool isNew, bool isArchived)
{
    CMintMeta meta;
...
void CzDIDTracker::Add(const CZerocoinMint& mint, bool isNew, bool isArchived)
{
    CMintMeta meta;
...
void CzDIDTracker::SetPubcoinUsed(const uint256& hashPubcoin, const uint256& txid)
{
...
void CzDIDTracker::SetPubcoinNotUsed(const uint256& hashPubcoin)
{
    if (!HasPubcoinHash(hashPubcoin))
        return;
    CMintMeta meta = GetMetaFromPubcoin(hashPubcoin);
...
void CzDIDTracker::RemovePending(const uint256& txid)
{
    uint256 hashSerial;
...
bool CzDIDTracker::UpdateStatusInternal(const std::set<uint256>& setMempool, CMintMeta& mint)
{
...
std::set<CMintMeta> CzDIDTracker::ListMints(bool fUnusedOnly, bool fMatureOnly, bool fUpdateStatus)
{
    CWalletDB walletdb(strWalletFile);
    if (fUpdateStatus) {
        ...
        for (auto& dMint : listDeterministicDB)
            Add(dMint);
        LogPrint("zero", "%s: added %d dzdid from DB\n", __func__, listDeterministicDB.size());
    }
